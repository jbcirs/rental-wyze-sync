...
def http_trigger_sync(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logger.info(f"req.params: {req.params}")
    delete_all_guest_codes = req.params.get('delete_all_guest_codes', 'false').lower() == 'true'
    logger.info(f"delete_all_guest_codes: {delete_all_guest_codes}")
...
def property_list(req: func.HttpRequest) -> func.HttpResponse:
    logger.info('HTTP trigger function processed a request get_property_list.')

    try:
        from hospitable import authenticate_hospitable, get_properties
        token = authenticate_hospitable()
        if not token:
            logger.info("Unable to authenticate with Hospitable API.")
            return

        properties = get_properties(token)
        if not properties:
            logger.info("Unable to fetch properties from Hospitable API.")
            return
